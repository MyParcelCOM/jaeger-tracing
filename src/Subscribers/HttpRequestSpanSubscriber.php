...
class HttpRequestSpanSubscriber
{
    ...
    {
        if (!$this->config->get('jaeger.enabled', false)) {
            return;
        }
    ...
    {
        return in_array($routeName, $this->config->get('jaeger.trace_routes', []), true);
    }
    ...
    {
        return "{$event->request->getMethod()} {$this->router->currentRouteAction()}";
    }
